...
const PROMPT_RESUMEN_ADMIN = "Objetivo: Generar un resumen diario profesional y conciso para el Administrador resaltando conteos, problemas, sugerencias y tareas; Parámetros: dias entre 1 y 7 indicando el numero de dias hacia atras; Formato de salida: para cada dia del mas antiguo al mas reciente mostrar DD/MM seguido de 1 Conteos realizados X; 2 Problemas reportados Usuario (Sucursal) breve descripcion; 3 Sugerencias recibidas Usuario (Sucursal) breve descripcion; 4 Tareas pendientes Usuario (Sucursal) breve descripcion; 5 Acciones recomendadas accion concreta con responsable y plazo; Si un dia no tiene actividad mostrar DD/MM sin registros relevantes; Restricciones: no inventar datos ni incluir registros fuera del rango, no usar parrafos largos solo listas numeradas y encabezados simples."

const PROMPT_RECEPCION_COMPRAS = "Objetivo: Guiar al usuario para registrar la recepcion de una compra. La IA intentará extraer fecha, total y proveedor de la factura cargada; si no puede, solicitá esos datos. Luego pedí transporte y faltantes, confirmá la información y ejecutá la herramienta correspondiente cuando todo esté completo.";

const PROMPT_TRASPASOS = "Objetivo: Registrar solicitudes de traspaso. Pedí al usuario la captura de pantalla del sistema Sicar y un comentario breve. Cuando tengas ambos datos llamá la herramienta correspondiente.";
